...
export function MessengerButton({
  ...
  function buildLink() {
    const msg = [
      ...
      `ID: ${plantId}`,
      `ลิงก์: ${plantUrl}`,
      selectedImageUrl ? `รูปที่เลือก: ${selectedImageUrl}` : "",
    ]
      .filter(Boolean)
      .join("\n");
  ...
  return (
    <a
      ...
      className="group flex w-full items-center justify-center gap-3 rounded-full bg-[#0866FF] px-8 py-4 text-sm font-medium text-white shadow-lg shadow-[#0866FF]/20 transition-all duration-300 ease-smooth hover:bg-[#0557e0] hover:shadow-[#0866FF]/30 active:scale-[0.98]"
    >
      {/* Messenger logo */}
      <svg
        width="20"
