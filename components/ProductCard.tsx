...
export function ProductCard({ plant, index, priority }: {
  ...
  return (
    <Link href={`/plants/${plant.id}`} className="group block" aria-label={plant.name}>
      <figure
        ...
        onTouchStart={() => hasMultiple && setImgIndex(i => (i + 1) % plant.images.length)}
      >
        {current ? (
          <Image
      ...
      <div className="mt-4 flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h3 className="font-display text-lg leading-tight text-ink dark:text-ink-dark">{plant.name}</h3>
          {plant.variety && (
            <p className="mt-0.5 truncate text-xs italic text-moss dark:text-cream/50">{plant.variety}</p>
          )}
        </div>
