...
const ADMIN_PASSWORD = process.env.NEXT_PUBLIC_ADMIN_KEY ?? "admin1234";

interface Props {
  plants: Plant[];
  onAdminMode?: (active: boolean) => void;
}

export function SearchBar({ plants, onAdminMode }: Props) {
  ...
  function highlight(text: string, q: string) {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark className="bg-forest/20 text-forest dark:bg-cream/20 dark:text-cream">{text.slice(idx, idx + q.length)}</mark>
    ...
    <div className="space-y-2">
      ...
      <div ref={containerRef} className="relative w-full max-w-xl mx-auto">
          ...
          <input
            ...
            className={`w-full rounded-full border py-3 pl-11 pr-5 text-sm outline-none transition-all ${
              isAdmin
                ...
                : "border-ink/15 bg-canvas text-ink placeholder:text-moss/60 focus:border-forest/50 focus:ring-2 focus:ring-forest/10 dark:border-cream/15 dark:bg-surface-dark dark:text-ink-dark dark:placeholder:text-cream/40"
            }`}
          />
          {query && (
            <button type="button" onClick={() => { setQuery(""); setOpen(false); inputRef.current?.focus(); }} className="absolute right-4 top-1/2 -translate-y-1/2 text-moss hover:text-ink dark:text-cream/40 dark:hover:text-cream">
        ...
        {open && results.length > 0 && (
          <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border border-ink/10 bg-canvas shadow-2xl shadow-ink/10 dark:border-cream/10 dark:bg-surface-dark dark:shadow-black/40">
            {results.map((plant, i) => (
              <button key={plant.id} type="button" onMouseEnter={() => setHighlighted(i)} onClick={() => goTo(plant)}
                className={`flex w-full items-center gap-3 px-4 py-3 text-left transition-colors ${i === highlighted ? "bg-forest/8 dark:bg-cream/8" : "hover:bg-ink/4 dark:hover:bg-cream/4"} ${i !== 0 ? "border-t border-ink/5 dark:border-cream/5" : ""}`}
              >
                {plant.images[0] && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={plant.images[0].thumbUrl ?? plant.images[0].url} alt="" className="h-10 w-10 flex-shrink-0 rounded-lg object-cover"/>
                )}
                ...
                {plant.price !== undefined && (
                  <p className="flex-shrink-0 text-xs tabular-nums text-moss dark:text-cream/50">฿{plant.price.toLocaleString()}</p>
                )}
              </button>
            ))}
          </div>
